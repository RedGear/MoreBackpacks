        ...
        while (!subFiles.empty())
        {
            ...
            try
            {
                if (file.isDirectory())
                {
                    ...
                    for (File addFile : addFiles) //make sure to add each file one at a time
                    {
                        ...
                        for (char working : line.toCharArray())
                        {
                            ...
                            switch (working)
                            {
                                ...
                                case ';': //denotes end of second number/end of set
                                    ...
                                    if (start == -1 && end == -1) //get all metas using default wildcard
                                    {
                                        addItem(packName, itemName);
                                    }

                                    if (start != -1 && end == -1) //get only one meta - start value
                                    {
                                        addItem(packName, itemName, start);
                                    }

                                    if (end != -1) //get all from start to end (if start is -1, change it to 0)
                                    {
